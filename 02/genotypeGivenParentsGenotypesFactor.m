...
numGenotype = nchoosek(numAlleles, 2) + numAlleles;
genotypeFactor.card = [numGenotype, numGenotype, numGenotype];

genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
% Replace the zeros in genotypeFactor.val with the correct values.
for idx = 1:prod(genotypeFactor.card)
    ass = IndexToAssignment(idx, genotypeFactor.card);
    cs = genotypesToAlleles(ass(1),:);
    ci = cs(1);
    cj = cs(2);
    p1s = genotypesToAlleles(ass(2),:);
    p1i = p1s(1);
    p1j = p1s(2);
    p2s = genotypesToAlleles(ass(3),:);
    p2i = p2s(1);
    p2j = p2s(2);
    ...
    if ci == cj
        ...
        genotypeFactor.val(idx) = prob1 * prob2;
        ...
        prob2 = length(choose2)/2;
        prod2 = prob1 * prob2;
        genotypeFactor.val(idx) = prod1 + prod2;
